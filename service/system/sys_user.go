...
func (userService *UserService) Register(u system.SysUser) (userInter system.SysUser, err error) {
	...
	if !errors.Is(global.GVA_DB.Where("stu_number = ?", u.StuNumber).First(&user).Error, gorm.ErrRecordNotFound) { // 判断学生名是否注册
		return userInter, errors.New("学生名已注册")
	}
...
func (userService *UserService) Login(u *system.SysUser) (userInter *system.SysUser, err error) {
	...
	var user system.SysUser
	err = global.GVA_DB.Where("stu_number = ?", u.StuNumber).Preload("Authorities").Preload("Authority").First(&user).Error
	if err == nil {
...
func (userService *UserService) SetUserInfo(req system.SysUser) error {
	return global.GVA_DB.Model(&system.SysUser{}).
		Select("updated_at", "stu_name", "header_img", "phone", "email", "sideMode", "enable").
		Where("id=?", req.ID).
		Updates(map[string]interface{}{
			"updated_at": time.Now(),
			"stu_name":   req.StuName,
